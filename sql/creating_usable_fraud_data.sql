...
Data Sources:
* yourorganization-prod.your_dataset.fraud_saas_history: Contains historical data of user activity and applied rules.
* yourorganization-prod.your_dataset.users: Provides user-related information such as user IDs and emails.
...
* 		Data Extraction and Transformation:
    ...
    * Parses JSON arrays (applied_rules) to extract specific rule attributes.
    * Retrieves additional user information such as user IDs from the users table.
    * Joins data with custom rules and transaction details.
* 		Join Conditions:
    * Joins fraud_saas_history with users based on matching email addresses.
    * Matches applied rules with custom rules using rule IDs.
...
SELECT 
  ...
  JSON_EXTRACT_SCALAR(s.device_details, '$.property1') AS device_property1,
  JSON_EXTRACT_SCALAR(s.device_details, '$.property2') AS device_property2,
  JSON_EXTRACT_SCALAR(s.email_details, '$.deliverable') AS email_deliverable,
  JSON_EXTRACT_SCALAR(s.email_details, '$.email') AS email_email,
  JSON_EXTRACT_SCALAR(s.email_details, '$.account_details.bukalapak.registered') AS email_bukalapak_registered,
  JSON_EXTRACT_SCALAR(s.ip_details, '$.ip') AS ip_ip,
  JSON_EXTRACT_SCALAR(s.ip_details, '$.account_details.microsoft.registered') AS ip_microsoft_registered,
...
LEFT JOIN UNNEST(JSON_EXTRACT_ARRAY(s.applied_rules)) AS applied_rule
LEFT JOIN
